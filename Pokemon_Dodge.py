...
def gameover(text,score):
    ...
    arq = open("MAX_SCORE.TXT", "r")
    last_score = int(arq.read())
    score1 = int(score)
    if score1 > last_score:
        arq = open("MAX_SCORE.TXT", "w")
        arq.write(str(score1))
        last_score=score1
    arq.close()
    ...
    TextSurf2, TextRect2 = text_objects("Your Score: "+str(score), mediumText)
    TextRect2.center = ((display_w/2),(display_h/1.40))
    gameDisplay.blit(TextSurf2, TextRect2)
    TextSurf3, TextRect3 = text_objects("Highscore: "+str(last_score), smallText)
    TextRect3.center = ((display_w/2),(display_h/1.11))
